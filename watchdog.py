...
(via state file /opt/job-agent/watchdog_state.json) to avoid spam.

Dependencies: stdlib only.

Deploy:
  scp watchdog.py root@77.42.84.12:/opt/job-agent/watchdog.py
  chmod +x /opt/job-agent/watchdog.py

Cron drop-in at /etc/cron.d/job-agent-watchdog:
  0 6,8,10,12,14,16,18,20,22 * * * root /usr/bin/python3 /opt/job-agent/watchdog.py >> /opt/job-agent/watchdog.log 2>&1

Env file /opt/job-agent/watchdog.env (600 perms):
...
# ── Config ─────────────────────────────────────────────────────────────────
AGENT_LOG        = Path("/opt/job-agent/job-agent/agent.log")
APPLICATIONS_DB  = Path("/opt/job-agent/job-agent/applications.db")
CONFIG_YAML      = Path("/opt/job-agent/job-agent/config.yaml")
STATE_FILE       = Path("/opt/job-agent/watchdog_state.json")
ENV_FILE         = Path("/opt/job-agent/watchdog.env")
